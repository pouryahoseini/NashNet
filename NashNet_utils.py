...
def computePayoff_2dBatch(game, equilibrium, pureStrategies_perPlayer, *argv):
    ...
    mixStrategies_p1 = K.reshape(mixStrategies_p1, (tf.shape(mixStrategies_p1)[0], tf.shape(mixStrategies_p1)[1], pureStrategies_perPlayer, 1))
    mixStrategies_p2 = tf.gather(equilibrium, 1, axis=2)
    mixStrategies_p2 = K.reshape(mixStrategies_p2, (tf.shape(mixStrategies_p2)[0], tf.shape(mixStrategies_p1)[1], 1, pureStrategies_perPlayer))
...
def computePayoff_np_2dBatch(game, equilibrium, pureStrategies_perPlayer, playerNumber):
    ...
    playerProbShape_grid = tuple(np.ones(playerNumber) + np.identity(playerNumber) * (pureStrategies_perPlayer - 1))
    playerProbShape = [tuple((tf.shape(mixStrategies_perPlayer[0])[0], tf.shape(mixStrategies_perPlayer[0])[1]) + tuple(playerProbShape_grid[pl])) for pl in
                       range(playerNumber)]
    mixStrategies_perPlayer = [K.reshape(mixStrategies_perPlayer[pl], playerProbShape[pl]) for pl in
                               range(playerNumber)]
...
def PayoffLoss_metric(game, payoff_Eq_function, pureStrategies_perPlayer, computePayoff_function, num_players):
    ...
    def Payoff(nashEq_true, nashEq_pred):
        _, loss_payoff_MSE = payoff_Eq_function(game, pureStrategies_perPlayer, nashEq_true, nashEq_pred, computePayoff_function, num_players)
...
def payoff_Eq_MSE(game, pureStrategies_perPlayer, nashEq_true, nashEq_pred, computePayoff_function, num_players):
    ...
    loss_Eq_MSE = tf.gather_nd(MSE_eq, tf.stack((tf.range(0, tf.shape(min_index)[0]), tf.cast(min_index, dtype = 'int32')), axis=1))
    ...
    payoff_true = computePayoff_function['computePayoff'](game, selected_trueNash, pureStrategies_perPlayer, num_players)
    payoff_pred = computePayoff_function['computePayoff'](game, tf.gather(nashEq_pred, 0, axis=1), pureStrategies_perPlayer, num_players)
...
def payoffv2_Eq_MSE(game, pureStrategies_perPlayer, nashEq_true, nashEq_pred, computePayoff_function, num_players):
    ...
    payoff_true = computePayoff_function['computePayoff_2dBatch'](game, nashEq_true, pureStrategies_perPlayer, num_players)
    payoff_pred = computePayoff_function['computePayoff'](game, tf.gather(nashEq_pred, 0, axis=1), pureStrategies_perPlayer, num_players)
...
# ********************************
def lossFunction_payoff_MSE(game, payoff_Eq_function, payoffToEq_weight, pureStrategies_perPlayer, computePayoff_function, num_players):
    '''
...
# ********************************
def lossFunction_payoff_Eq_weightedSum(game, payoff_Eq_function, payoffToEq_weight, pureStrategies_perPlayer, computePayoff_function, num_players):
    '''
    ...
    def payoff_Eq_weightedSum(nashEq_true, nashEq_pred):        
        ...
        loss_Eq_MSE, loss_payoff_MSE = payoff_Eq_function(game, pureStrategies_perPlayer, nashEq_true, nashEq_pred, computePayoff_function, num_players)
...
# ********************************
def lossFunction_payoff_Eq_multiplication(game, payoff_Eq_function, payoffToEq_weight, pureStrategies_perPlayer, computePayoff_function, num_players):
    '''
...
def hydra_payoff_Eq_MSE(game, pureStrategies_perPlayer, nashEq_true, nashEq_pred, computePayoff_function, num_players):
    ...
    #Run the matching with the true and predicted equilibria as proposer and proposed (in terms of the Deferred Acceptance Algorithm terms) each time
    eq_MSE_trueProposer, payoff_MSE_trueProposer = hydra_oneSided_payoff_Eq_MSE(nashEq_true, nashEq_pred, game, pureStrategies_perPlayer, computePayoff_function, num_players)
    eq_MSE_trueProposed, payoff_MSE_trueProposed = hydra_oneSided_payoff_Eq_MSE(nashEq_pred, nashEq_true, game, pureStrategies_perPlayer, computePayoff_function, num_players)
...
# ********************************
def hydra_oneSided_payoff_Eq_MSE(nashEq_proposer, nashEq_proposed, game, pureStrategies_perPlayer, computePayoff_function, num_players):
    '''
    ...
    payoff_proposer = computePayoff_function['computePayoff_2dBatch'](game, selected_proposerNash, pureStrategies_perPlayer, num_players)
    payoff_proposed = computePayoff_function['computePayoff_2dBatch'](game, nashEq_proposed, pureStrategies_perPlayer, num_players)
...
# ********************************
def hydra_payoffv2_Eq_MSE(game, pureStrategies_perPlayer, nashEq_true, nashEq_pred, computePayoff_function, num_players):
    '''
...
# ********************************
def hydra_oneSided_payoffv2_Eq_MSE(nashEq_proposer, nashEq_proposed, game, pureStrategies_perPlayer, computePayoff_function, num_players):
    '''
    ...
    #Compute payoffs given the equilibria
    payoff_proposed = computePayoff_function['computePayoff_2dBatch'](game, nashEq_proposed, pureStrategies_perPlayer, num_players)
    payoff_proposer = computePayoff_function['computePayoff_2dBatch'](game, nashEq_proposer, pureStrategies_perPlayer, num_players)
...
# ********************************
def build_model(num_players, pure_strategies_per_player, max_equilibria, optimizer, lossType, payoffLoss_type, enable_batchNormalization, payoffToEq_weight = None):
    '''
    ...
    loss_function, payoffLoss_function, computePayoff_function = chooseLossFunction(lossType, payoffLoss_type, num_players, enableHydra = False)
    
    # Compile the model
    model.compile(experimental_run_tf_function=False,
                  loss=loss_function(input_layer, payoffLoss_function, payoffToEq_weight, pure_strategies_per_player, computePayoff_function, num_players),
                  optimizer=optimizer,
                  metrics=[MSE, PayoffLoss_metric(input_layer, payoffLoss_function, pure_strategies_per_player, computePayoff_function, num_players)]
                  )
...
# ********************************
def build_hydra_model(num_players, pure_strategies_per_player, max_equilibria, optimizer, lossType, payoffLoss_type, enable_batchNormalization, hydra_shape, payoffToEq_weight = None):
    '''
    ...
    for headCounter in range(max_equilibria):
        ...
        for _ in range(1, num_players):
            last_layer_player[headCounter].append(tf.keras.layers.Dense(pure_strategies_per_player)(final_dense[headCounter]))
    ...
    for headCounter in range(max_equilibria):
        ...
        for playerCounter in range(1, num_players):
            softmax[headCounter].append(tf.keras.layers.Activation('softmax')(last_layer_player[headCounter][playerCounter]))

    # Create the output layer
    head_output = [None] * max_equilibria
    for headCounter in range(max_equilibria):
        head_output[headCounter] = tf.keras.layers.concatenate([softmax[headCounter][playerCounter] for playerCounter in range(num_players)])
    
    concatenated_heads = tf.keras.layers.concatenate([head_output[headCounter] for headCounter in range(max_equilibria)])
    output_layer = tf.keras.layers.Reshape((max_equilibria, num_players, pure_strategies_per_player))(concatenated_heads)

    # Create a keras sequential model from this architecture
    model = tf.keras.Model(inputs=input_layer, outputs=output_layer)

    # Determine which loss function to use
    loss_function, payoffLoss_function, computePayoff_function = chooseLossFunction(lossType, payoffLoss_type, num_players, enableHydra = True)

    # Compile the model
    model.compile(experimental_run_tf_function=False,
                  loss=loss_function(input_layer, payoffLoss_function, payoffToEq_weight, pure_strategies_per_player, computePayoff_function, num_players),
                  optimizer=optimizer,
                  metrics=[hydra_MSE, PayoffLoss_metric(input_layer, payoffLoss_function, pure_strategies_per_player, computePayoff_function, num_players)]
                  )
...
def printExamples(numberOfExamples, testSamples, testEqs, nn_model, examples_print_file, pureStrategies_per_player, lossType, payoffLoss_type, num_players, enable_hydra, payoffToEq_weight = None):
    ...
    for exampleCounter in range(numberOfExamples):
        #Computing the loss
        lossFunction_instance = lossFunction(exampleGame[exampleCounter], payoffLoss_function, payoffToEq_weight, pureStrategies_per_player, computePayoff_function, num_players)
...
#         return
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if (epoch % (self.max_epochs / self.num_cycles) == 0) and (epoch != 0):
            ...
            #Save the weights
            self.model.save_weights(self.save_dir.rstrip('/') + "/" + self.save_name + '_snapshot' + str(snpashot_num) + '_epoch' + str(epoch) + '.h5')
